...
export class PostgresStorageAdapter {
    ...
    async saveTriple(triple) {
        ...
        const result = await this.pool.query(`INSERT INTO knowledge_triples
       ...
       DO NOTHING`, [
        ...
        if ((result.rowCount ?? 0) === 0) {
            ...
            await this.pool.query(`UPDATE knowledge_triples
            SET confidence = LEAST(1.0, confidence + 0.1)
          WHERE tenant_id = $1
            AND subject   = $2
            AND predicate = $3
            AND object    = $4
            AND invalidated_at IS NULL`, [this.tenantId, triple.subject, triple.predicate, triple.object]);
        }
    ...
    async queryTriples(opts) {
        ...
        const { rows } = await this.pool.query(`SELECT id, subject, predicate, object, source_id, confidence, invalidated_at, created_at
...
export function resolvePostgresSsl(connectionString) {
    ...
    if (envOverride === 'off' || envOverride === 'false' || envOverride === '0')
        return false;
